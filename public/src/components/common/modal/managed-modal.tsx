...
const NewPasswordForm = dynamic(
  () => import('@components/auth/new-password-form')
);
const MatchOTPForm = dynamic(() => import('@components/auth/match-otp-form'));
...
export default function ManagedModal() {
  ...
  return (
    <Modal open={isOpen} onClose={closeModal}>
      ...
      {view === 'WRITE_PRODUCT_REVIEW' && <ReviewForm />}
      {view === 'MATCH_OTP' && <MatchOTPForm />}
      {view === 'NEW_PASSWORD' && <NewPasswordForm />}
    </Modal>
  );
}
