    ...
    <Listbox value={selectedItem} onChange={handleItemClick}>
      {({ open }) => (
        <div className="relative w-full font-body">
          <Listbox.Button className="relative flex items-center w-full p-4 border rounded cursor-pointer text-brand-dark md:p-5 ltr:text-left focus:outline-none border-border-base">
            {selectedItem?.icon}
            <span className="flex truncate items-center text-sm md:text-15px font-medium ltr:pl-2.5 relative">
              {t(selectedItem?.name)}
            </span>
            <span className="absolute inset-y-0 flex items-center pointer-events-none ltr:right-4 md:ltr:right-5">
              <FaChevronDown
          ...
          >
            ...
            >
                ...
                >
                  {({ selected, active }) => (
                    <span className="flex items-center">
                      {option?.icon}
                      <span
                        className={`block truncate ltr:pl-2.5 md:ltr:pl-3 ${
                      ...
                      {selected ? (
                        <span
                          className={`${active && 'text-amber-600'}
                                 absolute inset-y-0 ltr:left-0 flex items-center ltr:pl-3`}
                        />
                      ) : null}
                    </span>
                  )}
              ...
              >
                ...
                <span className="block truncate ltr:pl-2.5 md:ltr:pl-3">
                  {t('text-logout')}
                </span>
              </button>
            </Listbox.Options>
