...
import { IGetAllCities } from 'src/types';

export interface Props extends InputHTMLAttributes<HTMLInputElement> {
  className?: string;
  options: IGetAllCities[];
...
const SelectCities: React.FC<Props> = ({
  className = 'block',
  label,
  name,
  isLoading,
  ...
  return (
    <div className={className}>
      ...
      >
        ...
        {!isLoading ? (
          <>
            <option value="" className="font-medium" disabled selected>
              {label}
            </option>
            {options?.map((item: IGetAllCities, index: number) => (
              <option
                key={index}
                className={`${
                  value === item.cit_id.toString() ? 'font-bold' : 'font-normal'
                } block truncate`}
                value={item.cit_id}
              >
                {item.cit_name_en}
...
SelectCities.displayName = 'SelectCities';
