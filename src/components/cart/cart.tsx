      ...
      <div className="relative flex items-center justify-between w-full border-b ltr:pl-5 md:ltr:pl-7 border-border-base">
        ...
        <div className="flex items-center">
          {!isEmpty && (
            // @ts-ignore
            <button
              className="flex flex-shrink items-center text-15px transition duration-150 ease-in focus:outline-none text-brand-dark opacity-50 hover:opacity-100 ltr:-mr-1.5"
              aria-label={t('text-clear-all')}
              onClick={resetCart}
            >
              <DeleteIcon />
              <span className="ltr:pl-1">{t('text-clear-all')}</span>
            </button>
          )}
