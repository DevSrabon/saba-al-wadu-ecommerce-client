...
const social = [
  {
    id: 0,
    link: 'https://www.facebook.com/profile.php?id=1000092471028636',
    icon: <IoLogoFacebook />,
    className: 'facebook',
    title: 'text-facebook',
  },
...
export default function MobileMenu() {
    ...
    data.label && (
      <li className={`transition-colors duration-200 ${className}`}>
        <div className="relative flex items-center justify-between">
          <Link
            href={data.path}
            className="relative w-full py-4 transition duration-300 ease-in-out menu-item ltr:pl-5 md:ltr:pl-7 ltr:pr-4 text-brand-dark"
          >
          ...
          {hasSubMenu && (
            <div
              className="cursor-pointer w-full h-8 text-[17px] px-5 shrink-0 flex items-center justify-end text-brand-dark text-opacity-80 absolute ltr:right-0 top-1/2 transform -translate-y-1/2"
  ...
  const SubMenu = ({ dept, data, toggle, menuIndex }: any) => {
    ...
    return (
      <ul className={cn('mobile-sub-menu', dept > 2 && 'ltr:-ml-4')}>
        {data?.map((menu: any, index: number) => {
          ...
          return (
            <ListMenu
              ...
              key={menuName}
              menuIndex={index}
              className={cn(dept > 1 && 'ltr:pl-4', dept > 2 && 'ltr:pl-8')}
            />
          );
        })}
      </ul>
