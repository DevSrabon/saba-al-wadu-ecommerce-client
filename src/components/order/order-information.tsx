...
import Spinner, { SpinnerLoader } from '@components/ui/loaders/spinner/spinner';
...
export default function OrderInformation() {
  ...
  if (!isLoading && Object.keys(order).length === 0) {
    return (
      <div className="flex items-center justify-center px-4 py-4 my-6 text-sm border rounded-md border-border-base bg-fill-secondary lg:px-5 text-brand-dark md:text-base lg:mb-8">
        Order not found
      </div>
    );
  }
  return (
    <div className="py-16 xl:px-32 2xl:px-44 3xl:px-56 lg:py-20">
      <div className="flex items-center justify-start px-4 py-4 mb-6 text-sm border rounded-md border-border-base bg-fill-secondary lg:px-5 text-brand-dark md:text-base lg:mb-8">
        <span className="flex items-center justify-center w-10 h-10 rounded-full ltr:mr-3 lg:ltr:mr-4 bg-brand bg-opacity-20 shrink-0">
