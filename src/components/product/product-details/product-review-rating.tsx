...
import { IProductReview, IProducts, ISingleProductType } from 'src/types';
import { useProductReview } from '@rest/review';

const ProductReviewRating = ({ product }: { product: IProducts }) => {
  // const {
  //   query: { slug },
  // } = router;
  const { reviews } = useProductReview(product?.p_id?.toString() as string);
  const { t } = useTranslation('common');
  return (
    <div className="lg:flex">
      <div className="pt-2 lg:w-3/5">
        <h2 className="mb-8 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">
          Ratings & Reviews of {product?.p_name_en}
        </h2>
        {reviews.length > 0 ? (
