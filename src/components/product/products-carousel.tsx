...
const ProductsCarousel: React.FC<ProductsCarouselProps> = ({
  ...
  return (
    <div
      className={cn(
        'max-w-[1920px] overflow-hidden 4xl:overflow-visible px-4 md:px-6 lg:px-8 2xl:ltr:pl-10 2xl:ltr:pr-0 4xl:ltr:pr-10 mx-auto relative',
        className
      )}
    >
      ...
      {error ? (
        <div className="2xl:ltr:pr-10">
          <Alert message={error} />
        </div>
      ) : (
        <div
          className={`heightFull relative after-item-opacity xl:-mr-40 2xl:-mr-28 4xl:mr-0}`}
        >
          <Carousel
            breakpoints={carouselBreakpoint || breakpoints}
            className="-mx-1.5 md:-mx-2 xl:-mx-2.5 -mt-4"
            prevButtonClassName="ltr:-left-2 md:ltr:-left-1 lg:ltr:-left-2 xl:ltr:-left-2.5 2xl:ltr:left-5 -top-12 3xl:top-auto 3xl:-translate-y-2 4xl:-translate-y-10"
            nextButtonClassName="xl:lrt:translate-x-2.5 ltr:-right-2 xl:ltr:right-40 -top-12 3xl:top-auto transform 2xl:translate-x-0 3xl:-translate-y-2 4xl:-translate-y-10"
          >
            {loading && !products?.length ? (
              ...
              <>
                {products?.map((product: IProducts, idx) => (
                  <SwiperSlide
                    key={`${uniqueKey}-${idx}`}
                    className="px-1.5 md:px-2 xl:px-2.5 py-4"
                  >
