...
import { IGetAllSubCities } from 'src/types';

export interface Props extends InputHTMLAttributes<HTMLInputElement> {
  className?: string;
  options: IGetAllSubCities[],
...
const SelectSubCities: React.FC<Props> = (
  (
    {
      className = 'block',
      label,
      ...
      <div className={className}>
        ...
        >
          ...
          {!isLoading ? <>
            ...
            {options?.map((item: IGetAllSubCities, index: number) => (
              <option key={index} className={`${value === item.scit_id.toString() ? 'font-bold' : 'font-normal'
                } block truncate`} value={item.scit_id} >{item.scit_name_en}</option>
            ))}
...
SelectSubCities.displayName = 'SelectSubCities';
