...
import { IOrderResponseType, IOrders, IOrdersResponseType, IPurchaseOrder, IPurchaseOrderResponseType } from "src/types";
...
export function useOrders() {
  const { data, isLoading, error } = useQuery<IOrdersResponseType, Error>(
    [API_ENDPOINTS.PRODUCT_ORDER],
    () => client.orders.list(), {refetchOnWindowFocus: false})
  return {
